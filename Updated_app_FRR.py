...
def process_data(df_raw: pd.DataFrame):
    ...
    - df: cleaned
    - allcctu: all 0-24 rows
    - allcctu_sel: 0-24 & selected
    ...
    # Period as categorical in explicit order
    df['capacitybiddeliveryperiod'] = df['capacitybiddeliveryperiod'].astype(str)
    df['capacitybiddeliveryperiod'] = pd.Categorical(df['capacitybiddeliveryperiod'], categories=PERIODS, ordered=True)

    # Dynamic ymax per date (consider only 6x 4h periods)
    df_4h = df[df['capacitybiddeliveryperiod'].isin(PERIODS)]
...
def build_summary_and_costs(df: pd.DataFrame, allcctu_sel: pd.DataFrame):
    ...
    """
    # Summary by period/direction for the 6x 4-hour periods
    d4 = df[df['capacitybiddeliveryperiod'].isin(PERIODS)].copy()

    # Total submitted per period/direction (using offered volumes)
    agg_submitted = d4.groupby('capacitybiddeliveryperiod').agg(
